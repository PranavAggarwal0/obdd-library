<script>
	...
	async function doPost() {
		const res = await fetch('http://localhost:8080/disjunction', {
			method: 'POST',
			mode: 'cors',
...
{#if show_info}
	<p id="init">
		...
		third node that represents the disjunction of the two nodes. The two input nodes are coloured in
