<script>
	...
	async function doPost(filcon) {
		const res = await fetch('http://localhost:8080/build/dimacs', {
			method: 'POST',
			mode: 'cors',
...
{#if show_info}
	<p id="init">
		This page allows you to build an OBDD for input specified as a DIMACS file. For more information
		about this format, check out <a
			href="https://jix.github.io/varisat/manual/0.2.0/formats/dimacs.html">this page</a
...
{#if !show_info}
	<p id="init"><b>Get OBDD from a file in DIMACS format:</b></p>
	<br />
